...
void BGCSimple::Setup()
{
  ...
  for (const auto& region : regions) {
    sc_params_.push_back(Teuchos::rcp(
        new SoilCarbonParameters(num_pools_, sc_params.sublist(region))));
  }
  ...
  S_->Require<CompositeVector,CompositeVectorSpace>(key_, tag_next_, name_).SetMesh(mesh_)
  ...
  S_->Require<CompositeVector,CompositeVectorSpace>("surface-incoming_shortwave_radiation", tag_next_)
    .SetMesh(mesh_surf_)->AddComponent("cell", AmanziMesh::CELL, 1);
...
void BGCSimple::Initialize() {
  ...
  if (!leaf_biomass_field.initialized()) {
    ...
    if (plist_->isSublist("leaf biomass initial condition")) {
      ...
      if (leaf_biomass_field.initialized()) {
        // -- copy into PFTs
        Epetra_MultiVector& bio = *S_->GetPtrW<CompositeVector>("surface-leaf_biomass", tag_next_, name_)
...
void BGCSimple::CommitStep(double told, double tnew, const Tag& tag)
{
...
bool BGCSimple::AdvanceStep(double t_old, double t_new, bool reinit)
{
  ...
  Epetra_MultiVector& trans = *S_->GetW<CompositeVector>(trans_key_, tag_next_, name_)
      .ViewComponent("cell",false);
  Epetra_MultiVector& sw = *S_->GetW<CompositeVector>(shaded_sw_key_, tag_next_, name_)
      .ViewComponent("cell",false);
  Epetra_MultiVector& biomass = *S_->GetW<CompositeVector>("surface-total_biomass", tag_next_, name_)
      .ViewComponent("cell",false);
  Epetra_MultiVector& leafbiomass = *S_->GetW<CompositeVector>("surface-leaf_biomass", tag_next_, name_)
      .ViewComponent("cell",false);
  Epetra_MultiVector& csink = *S_->GetW<CompositeVector>("surface-c_sink_limit", tag_next_, name_)
      .ViewComponent("cell",false);
  Epetra_MultiVector& total_lai = *S_->GetW<CompositeVector>(total_lai_key_, tag_next_, name_)
      .ViewComponent("cell",false);
  Epetra_MultiVector& lai = *S_->GetW<CompositeVector>("surface-leaf_area_index", tag_next_, name_)
      .ViewComponent("cell",false);
  Epetra_MultiVector& total_transpiration = *S_->GetW<CompositeVector>("surface-veg_total_transpiration", tag_next_, name_)
      .ViewComponent("cell",false);
  ...
  const Epetra_MultiVector& qSWin = *S_->Get<CompositeVector>("surface-incoming_shortwave_radiation", tag_next_)
      .ViewComponent("cell",false);
  ...
  const Epetra_MultiVector& co2 = *S_->Get<CompositeVector>("surface-co2_concentration", tag_next_)
      .ViewComponent("cell",false);
